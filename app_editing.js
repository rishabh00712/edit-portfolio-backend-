...
// Only BREVO_API_KEY is required in .env. The "from" address defaults to
// OWNER_EMAIL, which must be a sender/email you have verified in Brevo.
// If Brevo rejects it, set BREVO_SENDER_EMAIL to a verified sender.
// Brevo transactional email (Node 18+ has global fetch).
// Only BREVO_API_KEY is required in .env. The "from" address defaults to
// OWNER_EMAIL, which must be a sender/email you have verified in Brevo.
// If Brevo rejects it, set BREVO_SENDER_EMAIL to a verified sender.
async function sendOtpEmail(to, otp) {
  if (!process.env.BREVO_API_KEY) {
    console.error("[OTP] FAILED - BREVO_API_KEY is missing in the environment variables.");
